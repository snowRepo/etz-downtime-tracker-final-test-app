...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'create_template') {
            ...
            $incidentTypeId = $_POST['incident_type_id'] ?? null;
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$templateName, $serviceId, $componentId, $incidentTypeId, $impactLevel, $description, $rootCause, $_SESSION['user_id']]);
            ...
            $stmt = $pdo->prepare("
                ...
                SET template_name = ?, service_id = ?, component_id = ?, incident_type_id = ?,
                    impact_level = ?, description = ?, root_cause = ?
                WHERE template_id = ?
            ");
            $stmt->execute([$templateName, $serviceId, $componentId, $incidentTypeId, $impactLevel, $description, $rootCause, $templateId]);
...
// Fetch all data
try {
    // Get all templates with service, component, and incident type names
    $templates = $pdo->query("
        SELECT t.*, 
               s.service_name,
...
<body class="relative min-h-screen">
        ...
        <main class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8">
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Create reusable templates for common
                        incidents</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
                        <div>
                            <p
                                class="text-sm font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-3">
                                Most Used</p>
                                ...
                                <?php foreach ($topTemplates as $idx => $top): ?>
                                    <div class="flex items-center justify-between mb-2">
                                        <span class="text-xs text-gray-700 dark:text-gray-300 truncate"
                                            title="<?= htmlspecialchars($top['template_name']) ?>">
                                            <?= ($idx + 1) ?>.
                                            <?= htmlspecialchars(substr($top['template_name'], 0, 20)) ?>
                                            <?= strlen($top['template_name']) > 20 ? '...' : '' ?>
                                        </span>
                                        <span
                                            class="text-xs font-semibold text-blue-600 dark:text-blue-400"><?= $top['usage_count'] ?></span>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <?php if ($template['service_name']): ?>
                                                    <span
                                                        class="text-sm text-gray-900 dark:text-white"><?= htmlspecialchars($template['service_name']) ?></span>
                                                <?php else: ?>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <?php if ($template['component_name']): ?>
                                                    <span class="text-sm text-gray-900 dark:text-white">
                                                        <?= htmlspecialchars($template['component_name']) ?>
                                                    </span>
                                                <?php else: ?>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <?php if ($template['incident_type_name']): ?>
                                                    <span
                                                        ...
                                                        <?= htmlspecialchars($template['incident_type_name']) ?>
                                                    </span>
                                                <?php else: ?>
                                                    <span class="text-xs text-gray-400 dark:text-gray-500 italic">Any Type</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <?php
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <form method="POST" class="inline" onchange="this.submit()">
                                                    ...
                                                    <label class="relative inline-flex items-center cursor-pointer">
                                                        ...
                                                        <span
                                                            class="ms-3 text-sm font-medium <?= $template['is_active'] ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400' ?>">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                                <button onclick='editTemplate(<?= json_encode($template, JSON_HEX_APOS) ?>)'
                                                ...
                                                <button
                                                    onclick="deleteTemplate(<?= $template['template_id'] ?>, '<?= addslashes(htmlspecialchars($template['template_name'])) ?>')"
    ...
    <div id="templateModal"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white" id="templateModalTitle">Create Template
                </h3>
            </div>
            <form method="POST" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="templateServiceId"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Service
                        </label>
                        <select name="service_id" id="templateServiceId"
                            onchange="filterComponents(this.value); filterIncidentTypes(this.value);"
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="templateIncidentTypeId"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Incident Type
                        </label>
                        <select name="incident_type_id" id="templateIncidentTypeId"
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
    ...
    <script>
        ...
        // Incident type filtering (mirrors component filtering)
        function filterIncidentTypes(serviceId) {
            ...
            // Reset type selection if it doesn't match the new service
        ...
        function showTemplateModal() {
            ...
            document.getElementById('templateIncidentTypeId').value = '';
        ...
        function editTemplate(template) {
            ...
            // Filter first so the correct options are visible, then set the values
            filterComponents(template.service_id || '');
            document.getElementById('templateComponentId').value = template.component_id || '';
            filterIncidentTypes(template.service_id || '');
            document.getElementById('templateIncidentTypeId').value = template.incident_type_id || '';
            document.getElementById('templateModal').classList.remove('hidden');
        }
