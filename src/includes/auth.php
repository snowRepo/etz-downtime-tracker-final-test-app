...
function login($usernameOrEmail, $password) {
    ...
    try {
        ...
        if ($user) {
            ...
            logActivity(null, 'login_failed', "Failed login attempt for: {$usernameOrEmail}", null, null, ['username_or_email' => $usernameOrEmail]);
        }
...
function getLoginUrl() {
    ...
    $publicPath = str_replace('\\', '/', $scriptPath);
    
    // If we're in public directory, login.php is in same directory
    if (strpos($publicPath, '/public') !== false) {
        return 'login.php';
    }
    
    // Otherwise, construct path to public/login.php
    return $publicPath . '/public/login.php';
}
