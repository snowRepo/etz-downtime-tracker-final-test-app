...
<nav class="bg-white dark:bg-gray-800 sticky top-0 z-50 border-b border-gray-200 dark:border-gray-700 transition-colors duration-200"
            ...
            <div class="flex items-center justify-between w-full">
                ...
                <div class="hidden md:flex space-x-1 absolute left-1/2 transform -translate-x-1/2">
                    <a href="<?= url('index.php') ?>"
                        class="<?php echo isActive('index.php'); ?> px-4 py-2 text-sm font-medium transition-colors duration-150">
                        Dashboard
                    </a>
                    <a href="<?= url('incidents.php') ?>"
                        class="<?php echo isActive('incidents.php'); ?> px-4 py-2 text-sm font-medium transition-colors duration-150">
                        Incidents
                    </a>
                    <a href="<?= url('sla_report.php') ?>"
                        class="<?php echo isActive('sla_report.php'); ?> px-4 py-2 text-sm font-medium transition-colors duration-150">
                        SLA
                    </a>
                    <a href="<?= url('analytics.php') ?>"
                    ...
                    if ($currentUser):
                        ...
                        <div class="hidden md:block relative" x-data="{ userMenuOpen: false }">
                            <button @click="userMenuOpen = !userMenuOpen" type="button"
                                class="flex items-center justify-center w-10 h-10 rounded-full bg-blue-600 text-white font-semibold hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
    ...
    <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="px-2 pt-2 pb-3 space-y-1">
            <a href="<?= url('index.php') ?>"
                class="<?php echo isMobileActive('index.php'); ?> block pl-3 pr-4 py-2.5 border-l-4 text-sm font-medium transition-colors duration-150">
                Dashboard
            </a>
            <a href="<?= url('incidents.php') ?>"
                class="<?php echo isMobileActive('incidents.php'); ?> block pl-3 pr-4 py-2.5 border-l-4 text-sm font-medium transition-colors duration-150">
                Incidents
            </a>
            <a href="<?= url('sla_report.php') ?>"
                class="<?php echo isMobileActive('sla_report.php'); ?> block pl-3 pr-4 py-2.5 border-l-4 text-sm font-medium transition-colors duration-150">
                SLA
            </a>
            <a href="<?= url('analytics.php') ?>"
